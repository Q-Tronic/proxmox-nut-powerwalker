...
Autor: **Q-Tronic**  
Wersja projektu: **1.0.0-rc3** (`VERSION` w repo) — release candidate do testów na fizycznym UPS
...
`1.0.0-rc3` jest kandydatem do pierwszego stabilnego wydania z poprawką wznowienia nieukończonej instalacji i pustego subdrivera. Statyczne CI może sprawdzić składnię, ShellCheck, YAML i inwarianty projektu, ale **nie zastępuje testu konkretnego firmware UPS**. Taga `v1.0.0` nie twórz przed przejściem testów sprzętowych `OL -> OB -> OL`, pełnego shutdownu i — jeżeli zostanie użyty — kontrolowanego power-cycle.

Jeżeli utworzysz techniczny tag `v1.0.0-rc3`, workflow GitHub oznaczy go automatycznie jako **pre-release**. Kanał `stable` korzysta z `/releases/latest`, więc RC nie stanie się przypadkiem wydaniem stabilnym.
